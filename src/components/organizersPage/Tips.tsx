



const Tips = () => {
    return (
        <>
        <div className="mb-16">
            <h1 className="mb-8">Поради</h1>  
            <div className="flex border-b-2 pb-6 items-center">
              <h1 className="text-transparent bg-clip-text bg-gradient-to-r from-[#12C2E9] to-[#C471ED] mr-[69px]">01</h1>
              <h4 className="w-[477px] mr-[69px] text-4xl">Створюйте привабливий опис події</h4>
              <p className="w-[615px] text-xl">Пишіть опис події, котрий розповідає про суть події, її унікальність, та чим вона може бути правблива для участників</p>
            </div>
            <div className="flex border-b-2 pb-6 items-center">
              <h1 className="text-transparent bg-clip-text bg-gradient-to-r from-[#12C2E9] to-[#C471ED] mr-[66px]">02</h1>
              <h4 className="w-[480px] mr-[69px] text-4xl ">Якість фото</h4>
              <p className="text-xl w-[648px]">Обирайте якісні фото, фото які будуть відображати атмосферу та тематику події,</p>
            </div>
            <div className="flex items-center">
              <h1 className="text-transparent bg-clip-text bg-gradient-to-r from-[#12C2E9] to-[#C471ED] mr-[66px]">03</h1>
              <h4 className="w-[477px] mr-[69px] text-4xl">Соціальні мережі</h4>
              <p className="text-xl">Поділитися подією зі своїми друзями легко</p>
            </div>
        </div>
        </>
    )
}


export default Tips